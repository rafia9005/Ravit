...
import { LucideBook, LucideUser, LucideMail, LucideLock, LucideLoader2, LucideAlertCircle } from "lucide-react";
...
export default function Register() {
  const { handleRegister, isLoading, error: authError, successMessage } = useRegister();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validate form data
      const validatedData: RegisterInput = registerSchema.parse(formData);
      
      // Call register (pass without confirmPassword since API doesn't need it)
      ...
      // If it's not a ZodError, the error is already set by useRegister hook
    } finally {
      ...
      <div className="w-full max-w-md">
         ...
         <Card className="border-zinc-200 shadow-lg dark:border-zinc-800">
           ...
           <CardContent className="space-y-6 pb-6">
               ...
               {(authError || oauthError || Object.keys(errors).length > 0) && (
            ...
            {/* Social Buttons */}
             <div className="grid grid-cols-2 gap-3">
                ...
                <Button 
                  ...
                  type="button" 
                  size="lg"
                  onClick={() => loginWithGitHub("register")}
                  disabled={isSubmitting || isLoading || oauthLoading}
                >
